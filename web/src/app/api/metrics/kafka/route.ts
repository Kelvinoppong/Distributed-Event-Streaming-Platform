...
import { getDemoKafkaMetrics } from "@/lib/demo-data";
...
export async function GET() {
  try {
    ...
    const [
      ...
      queryRange(PROM_QUERIES.kafkaP99Produce, fifteenMinAgo, now, step),
      queryInstant(PROM_QUERIES.kafkaMessagesInPerBroker),
    ]);

    const hasData = messagesIn.length > 0 || perBroker.length > 0;
    if (!hasData) return NextResponse.json(getDemoKafkaMetrics());
    ...
    return NextResponse.json({
      ...
      series: {
        ...
        p99Produce: toTimeSeries(p99ProduceRange),
      },
    ...
    return NextResponse.json(getDemoKafkaMetrics());
  }
}
